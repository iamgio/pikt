...
class ColorSchemePalette(schemeInputStream: InputStream) {
    ...
    private fun generateList(keys: List<Any>, graphics: Graphics2D, xOffset: Int) {
        keys.forEachIndexed { index, name ->
            ...
            val hex = properties.getValue(name)
            graphics.color = Color.decode("#$hex")
            graphics.fillOval(xOffset, y - CIRCLE_DIAMETER / 2, CIRCLE_DIAMETER, CIRCLE_DIAMETER)
            graphics.drawString(name.toString(), xOffset + 45, y + CIRCLE_DIAMETER / 4)
        }
    }
}